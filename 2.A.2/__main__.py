...
if __name__ == "__main__":
    ...
    with config_context(target_offload="gpu"):
        gs_model = SGDClassifier(loss='log_loss', penalty='elasticnet', random_state=42, max_iter=1000)
    ...
    epochs_range = history.index + 1
    best_epoch_idx = history['val_loss'].idxmin()
    best_loss = history.loc[best_epoch_idx, 'val_loss']
    best_acc = history.loc[best_epoch_idx, 'val_acc']
    best_mae = history.loc[best_epoch_idx, 'val_mae']
    ...
    # plot 3: mae
    ax3.plot(epochs_range, history['train_mae'], label='Training MAE')
    ax3.plot(epochs_range, history['val_mae'], label='Validation MAE')
    ...
    # best params: {'alpha': 0.007742636826811277, 'eta0': 0.005, 'l1_ratio': 1.0, 'learning_rate': 'adaptive', 'tol': 1e-05}
    # ce: 0.4980 | acc: 0.7922 | mae: 0.3337